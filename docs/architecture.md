...
graph TD
 subgraph ubuntu_server["Ubuntu Server<br>251GB RAM, 64 Cores<br>(Intel Xeon E5-4620 v2 @ 2.6GHz)"]
    ...
    subgraph web_services["Web Services"]
      nginx["Nginx"]
      launchpad["Datascience Launchpad (FastAPI/Uvicorn)"]
    end

    subgraph docker_env["Docker Environment"]
      docker_daemon["Docker Daemon"]
      container_1["RStudio/JupyterLab Container 1"]
      container_n["RStudio/JupyterLab Container N"]
    end
    ...
    docker_daemon --> container_n
    container_1 -->|stores data on| home_partition
    container_n -->|stores data on| home_partition
